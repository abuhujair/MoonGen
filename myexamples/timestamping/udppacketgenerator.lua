...
-- local us_ptr = ffi.new("int64_t[1]") -- Create a pointer to int64_t
-- local function addTimeStamp(pkt)
-- 	local s, us = gettimeofday()
-- 	us_ptr[0] = s*1000000+us
-- 	ffi.copy(pkt.payload, us_ptr, 8)
-- end
...
function loadSlave(queue, rxDev, size, flows, time, ports)
	...
	local mempool = memory.createMemPool(function(buf)
        ...
        local payload = createTimeStampedHttpResponse()
